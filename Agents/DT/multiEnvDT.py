    ...
    def _create_sequences(self):
        ...
        for episode in self.episodes:
            ...
            for t in range(ep_len - 1):  # Can't predict beyond the last step
                ...
                seq_target_actions = [step['action'] for step in episode[end_idx:end_idx+20]]
                ...
                if len(seq_target_actions) < len(seq_input_actions):
                    seq_target_actions = [0 for _ in range(len(seq_input_actions) - len(seq_target_actions))] + seq_target_actions
                elif len(seq_target_actions) > len(seq_input_actions):
                    seq_target_actions = seq_target_actions[:-(len(seq_target_actions) - len(seq_input_actions))]

                if len(seq_target_actions) != len(seq_input_actions):
                    print(len(seq_input_actions))
                    print(len(seq_target_actions))
...
def main():
    ...
    model_dir = "agents\\game_obs\\DT\\MultiGame\\Results\\MultiGame_DT_v2"
