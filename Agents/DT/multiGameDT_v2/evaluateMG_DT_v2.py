...
import logging
from trainMultiGameDT import MultiGameDecisionTransformer, GameConfig, DEVICE, unflatten_action
...
class MultiGameDTEvaluator:
    def __init__(self, model_path, game_configs_path):
        ...
        self.model = MultiGameDecisionTransformer(
            self.game_configs, hidden_size, n_layer, n_head, dropout
        ).to(DEVICE)
    ...
    def evaluate_online(self, env, game_name, target_rtg, num_episodes=10):
        ...
        for ep in range(num_episodes):
            ...
            while not done:
                ...
                # The model returns a single prediction of shape (1, action_dim).
                # We select the first element to get the logits of shape (action_dim).
                predicted_action_logits = action_preds[0].cpu().numpy()
...
def main():
    # Configuration
    # model_path = "agents\\game_obs\\DT\\MultiGame\\Results\\MultiGame_DT_v1\\final_model.pt"
    model_path = "agents\\game_obs\\DT\\MultiGame\\Results\\MG_DT_v2\\final_model.pt"
    game_configs_path = "agents\\game_obs\\DT\\MultiGame\\Results\\MultiGame_DT_v1\\game_configs.json"
    
    game = "pirates"  # "solid" or "pirates"
